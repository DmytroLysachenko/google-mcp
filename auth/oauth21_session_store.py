...
class OAuth21SessionStore:
    ...
    def get_credentials_with_validation(
        ...
        with self._lock:
            logger.info(
                "Credential lookup user=%s session=%s auth_token=%s",
                requested_user_email,
                session_id or "none",
                "present" if auth_token_email else "missing",
            )
...
def ensure_session_from_access_token(
    ...
    if email:
        ...
        logger.info("Access token received without user email; session not cached")
