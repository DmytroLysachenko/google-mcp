...
def _get_auth_context(
    ...
    try:
        ...
        logger.info(
            "[%s] Auth context user=%s via=%s session=%s",
            tool_name,
            authenticated_user or "none",
            auth_method or "none",
            mcp_session_id or "none",
        )
        logger.debug(
...
async def get_authenticated_google_service_oauth21(
    ...
    logger.info(
        "[%s] OAuth2.1 access_token=%s session=%s",
        tool_name,
...
def _extract_oauth21_user_email(
    ...
    if not authenticated_user:
        logger.warning("[%s] OAuth 2.1 missing authenticated_user_email", func_name)
        raise Exception(
